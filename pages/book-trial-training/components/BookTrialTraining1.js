...
import Radiobox from "components/include/Radiobox/Radiobox";
...
import { checkEmail, getListCourse } from "redux/actions/siteAction";
...
function BookTrialTraining1(props) {
  ...
  useEffect(() => {
    const newLstCourse = lstCourse.filter(
      (course) => course.min_age <= ageStudent && ageStudent <= course.max_age
    );
    ...
    if (siteReducer.type) {
      ...
      if (siteReducer.type === siteActionType.GET_LIST_COURSE_SUCCESS) {
        ...
        if (global.bookTraining && global.bookTraining.date_of_birth) {
          ...
          dispatch(
            courseStartDate({
              course_id: global.bookTraining.courseSelected.course_id,
            })
          );
    ...
    <div className="tab-1">
      ...
      <div className="wSelect2">
        ...
        <Select
          ...
          onChange={(option) => {
            setMessage(option.ms_trial === 1 ? TRIAL_MESSAGE : FREE_MESSAGE);
        ...
        <input
          ...
          onChange={(event) => {
            ...
            if (
              ...
              timer.debounce(
                dispatch(checkEmail({ email: event.target.value })),
                1000
              );
            }
          }}
          ...
          {emailError && availableEmail && (
            <Fragment>
              ...
              <a
                ...
                href={"https://www.parentarea.co/parent/reset-password"}
              >
      ...
      {courseSatisfied.length > 0 && siteSelected ? (
              ...
              {courseSatisfied.map((item, index) => (
                ...
                >
                  <Radiobox
                    ...
                    checked={item.course_id === courseSelected.course_id}
                  >
                    {item.day_of_week}
                  </Radiobox>
                  <label>
        ...
        <div>
          <p style={{ color: "red", fontSize: "30px" }}>{notAvailable}</p>
        </div>
      )}
      <div>
        <p>
