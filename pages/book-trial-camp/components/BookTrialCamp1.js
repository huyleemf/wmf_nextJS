...
function BookTrialCamp1(props) {
  ...
  useEffect(() => {
    if (siteReducer.type) {
      if (siteReducer.type === siteActionType.GET_SITE_HAS_CAMP_SUCCESS) {
        ...
        if (global.bookCamp) {
          let _currentSite = siteReducer.data.lstSite.filter(function (site) {
            return site.ms_id === parseInt(global.bookCamp.siteId);
          });
  ...
  return (
    <div className="tab-1">
      ...
      <label>
        It only takes three minutes to create an account and make a booking.
      </label>
      <p>
                  ...
                  <div className="classInfo">
                    <Checkbox
                      ...
                      onChange={() => {
                        if (fullCourseSelect) {
                          setTotalPrice(0);
                          setDateSelect(dateSelect.map(() => false));
                        } else {
                          setTotalPrice(
                            parseFloat(courseSelected.course_price)
                          );
                          setDateSelect(dateSelect.map(() => true));
                        }
                        setFullCourseSelect(!fullCourseSelect);
                      }}
                    />
                    <label>Full Course ({courseSelected.course_title})</label>
                  </div>
              ...
              dateSelect.map((item, index) => {
                if (item) {
                  if (dateCourseSelect[index] === 0) {
                    _lstDate.push(eventDate[index].date + " - Full day");
                    _lstPrice.push(courseSelected.single_day_price);
                    eventDate[index].time.map((it, idx) => _dates.push(it.id));
                  } else {
                    _lstDate.push(eventDate[index].date + " - Half day");
                    _lstPrice.push(courseSelected.half_day_price);
      ...
      <div>
        ...
        <p>
          By clicking above, you agree that we may process your information in
